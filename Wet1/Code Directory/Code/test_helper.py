...
def memm_viterbi_untagged(all_tags, file_path, weights,feature_ids,model):
    """
       ...
       """
    final_tags = []
    prediction_file_path = 'comp_m'+str(model)+'_319010534.wtag'
    ...
    with open(file_path) as f:
        for idx, line in enumerate(f):
            ...
            underscore_array = ['_'] * len(splited_words[2:-2])
            line_txt_file = list(np.core.defchararray.add(list(np.core.defchararray.add(splited_words[2:-2], underscore_array)), line_tags_strings))
            line_txt_file = ' '.join(line_txt_file)
...
def memm_viterbi_iter_untagged(PI, PIbp, all_tags, k, all_words, feature_ids, weights):
    ...
    PI, PIbp = memm_viterbi_iter_untagged(PI, PIbp, all_tags, k+1, all_words, feature_ids, weights)
    return PI, PIbp
