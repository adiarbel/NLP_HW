...
def represent_input_with_features(history, features_id):
    ...
    if (word, ctag) in features_id.capitalized_tags_dict:
        features.append(features_id.capitalized_tags_dict[(word, ctag)])
...
def calc_objective_per_iter(w_i, lam, all_features, sum_of_real_features, all_tags, ids):
    """
    ...
    expected_counts = 0
    i = 0
    temp_feature = np.zeros((1, ids.n_total_features))
    for exp_term_Xi in exp_terms:
        ...
        for feature, exp_term in exp_term_Xi:
            ...
            temp_feature[feature] = np.ones(len(feature))
            expected_counts += temp_feature*exp_term/sum_curr_opt_features
...
def train(path,threshold):
    ...
    args = (lam, all_features, sum_of_real_features, all_tags, feature2id)

    w_0 = np.zeros(feature2id.n_total_features, dtype=np.float32)
    optimal_params = fmin_l_bfgs_b(func=calc_objective_per_iter, x0=w_0, args=args, maxiter=60, iprint=1)
    ...
    weights_path = 'train2_weights/trained_weights_data_i.pkl'  # i identifies which dataset this is trained on
